...
class AutoUpdateGithub extends AutoUpdate {
	...
	protected function get_plugin_sections() {
		...
		if ( ! is_wp_error( $response ) ) {
			...
			$urls[ __( 'Releases', 'wp-sortables' ) ] = sprintf( '%s/releases', $base_url );
			if ( $repoinfo->has_wiki ) {
				$urls[ __( 'Wiki', 'wp-sortables' ) ] = sprintf( '%s/wiki', $base_url );
			}
			if ( $repoinfo->has_issues ) {
				$urls[ __( 'Issues', 'wp-sortables' ) ] = sprintf( '%s/issues', $base_url );
			}
			if ( $repoinfo->license ) {
		...
		if ( ! is_wp_error( $response ) ) {
			$sections[ __('Description','wp-sortables') ] = wp_remote_retrieve_body($response);
		}

		if ( ! empty( $urls_section ) ) {
			$sections[ __( 'Links', 'wp-sortables' ) ] = $urls_section;
		}
		...
		if ( ! is_wp_error( $response ) ) {
			$sections[ __('Notes','wp-sortables') ] = wp_remote_retrieve_body( $response );
		}
